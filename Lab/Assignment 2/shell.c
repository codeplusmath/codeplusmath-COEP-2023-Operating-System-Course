...
// Tokenization function: tokens the input stream and separates them on " "
bool token(char *string, char **ret) {
...
int main(void){
	...
	while (should_run) {
		...
		if(strcmp(scanned_input, "hist") == 0) {
		...
		if(scanned_input[0] == '!') {
			...
			if(scanned_input[1] == '!') { // !! Case : Execute most recent
				...
				add_to_array(past_com[0]);
				
				char s1[MAX_LINE];
				strcpy(s1,past_com[0]);
				has_ampersand = token(s1, args);
				
				char com[MAX_LINE];
				strcpy(com,past_com[0]);
				has_ampersand = token(com, args);
			}
			...
			has_ampersand = token(scanned_input, args);
		}
		...
		// If the exit or CTRL+D(-1) statement is found 
